...
public class ReservationAdminController {
    ...
    public String newForm(@RequestParam(value = "voyageVehiculeId", required = false) Integer voyageVehiculeId,
        ...
        if (clientId != null)
            clientService.findById(clientId).ifPresent(reservation::setClient);
        if (etat != null)
        ...
        // Si un voyage est sélectionné, générer le plan de sièges et récupérer les
        ...
        if (voyageVehiculeId != null) {
            ...
            if (vv != null && vv.getVehicule() != null) {
                ...
                java.util.List<com.itu.taxibrousse.entity.Reservation> existing = reservationService
                        .findByVoyageVehicule(voyageVehiculeId);
                for (com.itu.taxibrousse.entity.Reservation r : existing) {
                    java.util.List<com.itu.taxibrousse.entity.ReservationDetails> details = reservationDetailsService
                            .findByReservation(r.getIdReservation());
                    for (com.itu.taxibrousse.entity.ReservationDetails d : details) {
                        if (d.getNumPlace() != null)
                            reservedSeats.add(d.getNumPlace());
                        System.out.println("Reserved seat: " + d.getNumPlace());
                    }
                }
                ...
                try {
                    java.util.List<com.itu.taxibrousse.entity.ClasseSiegeVehicule> mapped = csService
                            .findByVehicule(vv.getVehicule().getIdVehicule());
        ...
        // Etats (HTML badges) for the select; computed server-side to avoid static
        // access in Thymeleaf
    ...
    public String editForm(@PathVariable Integer id, Model model, RedirectAttributes redirectAttributes) {
        return reservationService.findById(id)
                .map(reservation -> {
                    ...
                    etatOptions.put(-1, Reservation.getEtatLibelle(-1));
                    model.addAttribute("etatOptions", etatOptions);
                    // Generate seat layout with reserved seats for the voyage, excluding this
                    // reservation's own seats
                    String seatLayoutHtml = null;
                    int capacity = 0;
                        ...
                        if (vv != null && vv.getVehicule() != null) {
                            ...
                            try {
                                java.util.List<com.itu.taxibrousse.entity.ClasseSiegeVehicule> mapped = csService
                                        .findByVehicule(vv.getVehicule().getIdVehicule());
                                for (com.itu.taxibrousse.entity.ClasseSiegeVehicule m : mapped) {
                                    if (m.getNumPlace() != null && m.getClasse() != null
                                            && m.getClasse().getIdClasse() != null) {
                                        seatToClass.put(m.getNumPlace(), m.getClasse().getIdClasse());
                                    }
                                }
                            } catch (Exception ignore) {
                            }
                            seatLayoutHtml = seatLayoutService.generateSeatLayoutHtml(capacity,
                                    new java.util.ArrayList<>(reservedSeats), seatToClass);
                        }
                    }
                    model.addAttribute("seatLayoutHtml", seatLayoutHtml);
                    model.addAttribute("capacity", capacity);
    ...
    public String view(@PathVariable Integer id, Model model, RedirectAttributes redirectAttributes) {
        return reservationService.findById(id)
                .map(reservation -> {
                    model.addAttribute("reservation", reservation);
                    java.util.List<com.itu.taxibrousse.entity.ReservationDetails> details = reservationDetailsService
                            .findByReservation(reservation.getIdReservation());
                    // compute per-detail montant using ReservationDetailsService helper
                    if (details != null) {
                        for (com.itu.taxibrousse.entity.ReservationDetails d : details) {
                            try {
                                if (d.getMontant() == null) {
                                    java.math.BigDecimal calc = reservationDetailsService.getMontantCalculated(d,
                                            reservation.getVoyageVehicule());
                                    if (calc != null)
                                        d.setMontant(calc.doubleValue());
                                }
                            } catch (Exception ignore) {
                            }
                        }
    ...
    public String save(@ModelAttribute Reservation reservation,
        ...
        try {
            ...
            java.util.List<com.itu.taxibrousse.entity.ReservationDetails> existing = reservationDetailsService
                    .findByReservation(saved.getIdReservation());
            ...
            // create details from mapping if provided, otherwise fallback to simple
            // selectedSeats
            ...
            // Try parsing seat->ClasseCategorie mapping; if it's empty JSON ("{}") then
            // ignore it and fall back to other inputs (selectedSeatsByCategory / selectedSeats).
            java.util.Map<String, Object> parsedByClasse = null;
            boolean hasParsedByClasse = false;
            if (selectedSeatsByClasseCategorie != null && !selectedSeatsByClasseCategorie.trim().isEmpty()) {
                try {
                    System.out.println("tafidira anaty try");
                    System.out.println("selectedSeatsByClasseCategorie=" + selectedSeatsByClasseCategorie);
                    System.out.println("selectedSeatsByCategory=" + selectedSeatsByCategory);
                    System.out.println("selectedSeats=" + selectedSeats);
                    System.out.println("montantParCategorie=" + montantParCategorie);
                    com.fasterxml.jackson.databind.ObjectMapper mapper = new com.fasterxml.jackson.databind.ObjectMapper();
                    parsedByClasse = mapper.readValue(selectedSeatsByClasseCategorie, java.util.Map.class);
                    System.out.println("parsedByClasse=" + parsedByClasse);
                    if (parsedByClasse != null && !parsedByClasse.isEmpty()) {
                        hasParsedByClasse = true;
                    } else {
                        logger.debug("selectedSeatsByClasseCategorie parsed empty, falling back to other inputs");
                    }
                } catch (Exception ex) {
                    logger.debug("Could not parse selectedSeatsByClasseCategorie: {}", ex.getMessage());
                }
            }

            if (hasParsedByClasse) {
                for (java.util.Map.Entry<String, Object> e : parsedByClasse.entrySet()) {
                    try {
                        ...
                        if (v != null) {
                            try {
                                idClasseCategorie = Integer.valueOf(String.valueOf(v));
                            } catch (NumberFormatException ignore) {
                                // ignore
                            }
                        }
                        System.out.println("seatNum=" + seatNum + " idClasseCategorie=" + idClasseCategorie);
                        ...
                        if (idClasseCategorie != null)
                            chosenCC = classeCategorieService.findById(idClasseCategorie).orElse(null);
                            // Compute montant using same helper as details page so rules match exactly
                            Double montant = null;
                            try {
                                // Ensure we pass a fully-initialized VoyageVehicule (with voyage/trajet/prix)
                                com.itu.taxibrousse.entity.VoyageVehicule vv = null;
                                if (saved.getVoyageVehicule() != null && saved.getVoyageVehicule().getIdVoyageVehicule() != null) {
                                    vv = voyageVehiculeService.findById(saved.getVoyageVehicule().getIdVoyageVehicule()).orElse(saved.getVoyageVehicule());
                                } else {
                                    vv = saved.getVoyageVehicule();
                                }
                                com.itu.taxibrousse.service.ReservationService.PriceResult pr = reservationService.computePriceForSeatWithSource(chosenCC, (chosenCC != null ? chosenCC.getCategorie() : null), vv);
                                if (pr != null && pr.price != null) {
                                    montant = pr.price.doubleValue();
                                    System.out.println("1- Computed price for seat " + seatNum + " source=" + pr.source + " price=" + pr.price);
                                }
                            } catch (Exception ignore) {}
                        rd.setMontant(montant);
                        rd.setEtat(saved.getEtat() != null ? saved.getEtat() : 1);
                ...
                try {
                    com.fasterxml.jackson.databind.ObjectMapper mapper = new com.fasterxml.jackson.databind.ObjectMapper();
                    java.util.Map<String, Object> parsed = mapper.readValue(selectedSeatsByCategory,
                            java.util.Map.class);
                    ...
                    if (parsedAsCatToList) {
                        ...
                        for (Object entryObj : parsed.entrySet()) {
                            ...
                            try {
                                parsedCatId = Integer.valueOf(catKey);
                            } catch (NumberFormatException ignore) {
                            }
                            ...
                            for (Object seatObj : seats) {
                                try {
                                    ...
                                    if (catId != null) {
                                        ...
                                        Integer vehiculeId = saved.getVoyageVehicule() != null
                                                && saved.getVoyageVehicule().getVehicule() != null
                                                        ? saved.getVoyageVehicule().getVehicule().getIdVehicule()
                                                        : null;
                                        com.itu.taxibrousse.entity.ClasseCategorie found = null;
                                        if (vehiculeId != null) {
                                            java.util.Optional<com.itu.taxibrousse.entity.ClasseSiegeVehicule> seatMapOpt = csService
                                                    .findByVehiculeAndNumPlace(vehiculeId, seatNum);
                                            com.itu.taxibrousse.entity.ClasseSiegeVehicule seatMap = seatMapOpt
                                                    .orElse(null);
                                            if (seatMap != null && seatMap.getClasse() != null
                                                    && seatMap.getClasse().getIdClasse() != null) {
                                                final Integer expectedClasseId = seatMap.getClasse().getIdClasse();
                                                found = classeCategorieService
                                                        .findByClasseAndCategorie(expectedClasseId, catId).orElse(null);
                                            }
                                        }
                                        if (found == null) {
                                            // fallback: any ClasseCategorie for this categorie
                                            java.util.List<com.itu.taxibrousse.entity.ClasseCategorie> list = classeCategorieService
                                                    .findByCategorie(catId);
                                            if (!list.isEmpty())
                                                found = list.get(0);
                                        }
                                        chosenCC = found;
                                    }
                                    // compute price using ClasseCategorie only (do not persist a separate Categorie
                                    ...
                                    try {
                                        ...
                                        if (pr != null && pr.price != null) {
                                            montant = pr.price.doubleValue();
                                            System.out.println("Computed price for seat " + seatNum + " source=" + pr.source + " price=" + pr.price);
                                        }
                                    } catch (Exception ignore) {}
                                    rd.setMontant(montant);
                                    rd.setEtat(1);
                                    ...
                                    if (chosenCC != null) {
                                        if (chosenCC.getIdClasseCategorie() != null) {
                                            chosenCC = classeCategorieService.findById(chosenCC.getIdClasseCategorie())
                                                    .orElse(chosenCC);
                                        }
                                        logger.debug("Assigning ClasseCategorie id={} categorieId={}",
                                                chosenCC.getIdClasseCategorie(),
                                                chosenCC.getCategorie() != null
                                                        ? chosenCC.getCategorie().getIdCategorie()
                                                        : null);
                                        rd.setClasseCategorie(chosenCC);
                                    }
                                    reservationDetailsService.save(rd);
                        ...
                        for (java.util.Map.Entry<String, Object> e : map.entrySet()) {
                            try {
                                ...
                                if (v != null) {
                                    try {
                                        parsedCatId = Integer.valueOf(String.valueOf(v));
                                    } catch (NumberFormatException ignore) {
                                    }
                                }
                                final Integer catId = parsedCatId;
                                ...
                                if (chosenCC != null) {
                                    ...
                                    logger.debug("Assigning ClasseCategorie id={} categorieId={}",
                                            chosenCC.getIdClasseCategorie(),
                                            chosenCC.getCategorie() != null ? chosenCC.getCategorie().getIdCategorie()
                                                    : null);
                                    rd.setClasseCategorie(chosenCC);
                                }
                                reservationDetailsService.save(rd);
                ...
                for (String p : parts) {
                    ...
                    try {
                        ...
                        java.math.BigDecimal price = reservationService.computePriceForSeat(null, null,
                                saved.getVoyageVehicule());
    ...
    /**
     * AJAX endpoint: get computed price for a voyageVehicule given optional
     * classeId and categorieId.
    ...
    public java.util.Map<String, Object> getPrice(@RequestParam("voyageVehiculeId") Integer voyageVehiculeId,
        ...
        try {
            com.itu.taxibrousse.entity.VoyageVehicule vv = voyageVehiculeService.findById(voyageVehiculeId)
                    .orElse(null);
            com.itu.taxibrousse.entity.Categorie cat = null;
            if (categorieId != null)
                cat = categorieService.findById(categorieId).orElse(null);

            // Return tariffs for all classes/categories for the selected voyageVehicule
            // (ignore form selections)
            java.util.List<com.itu.taxibrousse.service.ReservationService.PriceResult> options = reservationService
                    .collectAllOptionsForVoyageVehicule(vv);
            ...
            for (com.itu.taxibrousse.service.ReservationService.PriceResult p : options) {
                ...
                m.put("price",
                        p.price != null ? p.price.setScale(2, java.math.RoundingMode.HALF_UP).toString() : "0.00");
                m.put("source", p.source != null ? p.source : "");
                m.put("label", p.label != null ? p.label : "");
                opts.add(m);
            }
            resp.put("options", opts);
            // keep a convenience "price" and "source" (first option) for backward
            // compatibility
            if (!opts.isEmpty()) {
